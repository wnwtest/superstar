...
def check_commands():
    for cmd in ['repo', 'git']:
        if not command_exists(cmd):
            raise RepoTagError("错误: {cmd} 未安装。请安装后再运行此脚本。".format(cmd))
def check_repo_dir():
...
def create_manifest(tag_name):
    ...
    if not os.path.exists(tag_dir):
        os.makedirs(tag_dir)
    manifest_file = os.path.join(tag_dir, "{}.xml".format(tag_name))
    subprocess.check_call(['repo', 'manifest', '-o', manifest_file])
    return os.path.join("{}.xml".format(tag_name))
def push_changes(tag_name, manifest_file, remote_branch='master'):
    ...
    try:
        ...
        if not result.stdout:
            raise RepoTagError("远程分支 '{}' 不存在".format(remote_branch))
        subprocess.check_call(['git', 'add', manifest_file])
        subprocess.check_call(['git', 'commit', '-m', 'Add manifest for tag {}'.format(tag_name)])
        subprocess.check_call(['git', 'push', 'origin', 'HEAD:{}'.format(remote_branch)])
    except subprocess.CalledProcessError as e:
        raise RepoTagError("Git 操作失败: {}".format(str(e)))
    finally:
        # 返回到原始目录
...
def create_and_push_tag(tag_name, push=False, remote_branch='master'):
    ...
    try:
        ...
        print("新的 manifest 文件已生成: {}".format(manifest_file))
        if push:
        ...
        raise RepoTagError("未预期的错误: {}".format(str(e)))
