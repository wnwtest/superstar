...
def apply_prefixes(project, name_prefix, path_prefix):
    ...
    if path_prefix and path_prefix.strip():  # 只有在path_prefix非空时才添加
...
def process_single_xml(input_path, name_prefix, path_prefix):
    """处理单个XML文件"""
    print("Processing XML file: '{}'".format(input_path))
    ...
    for project in original_root.findall('project'):
        ...
        if project.find('linkfile') is not None or project.find('copyfile') is not None:
            # 特殊项目（不包含链接和复制文件） 使用remove-project
    ...
    for operation in operations:
        ...
        for project in original_root.findall('project'):
            ...
            if valid_linkfiles or valid_copyfiles:
                ...
                # 全局特殊项目（不包含链接和复制文件）
                special_project_elem = ET.Element('remove-project')
...
def main():
    ...
    parser.add_argument('--config', help='YAML configuration file', default='repo.yaml')
    ...
    baseline_product = config.get('baseline', {}).get('product', '')


    # 处理每个XML文件
    for operation in config.get('operations', []):
        input_file = operation.get('file')
        name_prefix = baseline_product + "/" + operation.get('name_prefix', '')  # 结合 baseline 的 product
        path_prefix = operation.get('path_prefix', '')
    ...
    # 生成全局特殊项目文件
    process_global_special_projects(args.input_dir, config.get('operations', []))
